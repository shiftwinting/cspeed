 ...
 */
int deal_with_request(char *url, zval *object, char *method_name, zend_execute_data *execute_data, zval *return_value)
{
    zval retval;
...
SPEED_METHOD(App, get)
{
    ...
    if (Z_TYPE_P(function) == IS_OBJECT) {
        if ( SPEED_METHOD_IN_OBJECT(function, "init") ) {
            ...
            if (!deal_with_request(url, function, "init", execute_data, return_value)) {
                RETURN_FALSE
            }
        } else {
            /* Call the anonymous function*/
            if (!deal_with_request(url, function, NULL, execute_data, return_value)) {
                RETURN_FALSE
            }
        }
    } else {
